...
public class UIPanel : UIRect
{
	...
	public bool dontRoundInClip = false;
	...
	public void InvalideMatrix(bool includeChildren)
	{
		mMatrixFrame = -1;
		...
		if (includeChildren)
		{
			for (int i = 0, imax = list.Count; i < imax; ++i)
			{
				UIPanel p = list[i];
				if (p != this && p.parentPanel == this)
					p.InvalideMatrix(includeChildren);
			}
		}
	}
	...
	void UpdateDrawCalls (int sortOrder)
	{
		...
		if (isUI)
		{
			...
			if (clipping != UIDrawCall.Clipping.None && !dontRoundInClip)
			{
